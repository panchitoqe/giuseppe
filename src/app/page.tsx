...
type Order = { id: string; customer: string; phone: string; product: string; quantity: number; total: number; deliveryStatus: DeliveryStatus; paymentStatus: PaymentStatus; deliveryCost: number; deliveryPaymentStatus: PaymentStatus; destination: string; date: string };
type SupabaseOrder = { order_code: string; customer_name: string; phone: string; product_name: string; quantity: number; total: number; delivery_status: "pending" | "delivered"; payment_status: "pending" | "paid"; delivery_cost: number; delivery_payment_status: "pending" | "paid"; destination: string; created_at: string };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.from("orders").select("order_code, customer_name, phone, product_name, quantity, total, delivery_status, payment_status, delivery_cost, delivery_payment_status, destination, created_at").order("created_at", { ascending: false }).then(({ data, error }) => {
      if (error || !data) return;
      setOrders((data as SupabaseOrder[]).map((order) => ({ id: order.order_code, customer: order.customer_name, phone: order.phone, product: order.product_name, quantity: order.quantity, total: Number(order.total), deliveryStatus: order.delivery_status === "delivered" ? "Entregado" : "Pendiente", paymentStatus: order.payment_status === "paid" ? "Pagado" : "Pendiente", deliveryCost: Number(order.delivery_cost), deliveryPaymentStatus: order.delivery_payment_status === "paid" ? "Pagado" : "Pendiente", destination: order.destination, date: formatOrderDate(order.created_at) })));
    });
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const newOrder: Order = { id: `#GP-${1050 + orders.length}`, customer: name, phone, product, quantity, total: quantity * price, deliveryStatus: "Pendiente", paymentStatus: "Pendiente", deliveryCost: hasDeliveryCharge ? deliveryCost : 0, deliveryPaymentStatus: hasDeliveryCharge ? deliveryPaymentStatus : "Pendiente", destination: destination === "arequipa" ? "Arequipa" : "Provincia", date: "Ahora" };
